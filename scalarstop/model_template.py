...
class KerasModelTemplate(ModelTemplate):
    """Describes machine learning model architectures and hyperparameters. Used to generate new machine learning model objects that are passed into :py:class:`~scalarstop.model.Model` objects."""  # pylint: disable=line-too-long

    _model = None

    def __repr__(self) -> str:
        return f"<sp.KerasModelTemplate {self.name}>"

    @property
    def custom_objects(self) -> Dict[str, Type]:
        """
        Returns a dictionary of string class names to Python class instances,
        representing custom functions and classes used to build a Keras model architecture.

        When a Keras model is serialized into the TensorFlow SavedModel format,
        the SavedModel contains the TensorFlow computational graph generated by
        the layers in the model, but the actual Python classes or functions used
        to build custom layers are not saved.

        When deserializing a Keras SavedModel, you can provide a dictionary mapping
        of the custom classes and functions used to build the computational graph.

        The TensorFlow documentation has a guide on
        `how to handle custom objects <https://www.tensorflow.org/guide/keras/save_and_serialize#custom_objects>`_
        when loading/saving Keras models to/from the filesystem.
        """
        raise IsNotImplemented("KerasModelTemplate.custom_objects")
    ...
    def new_model(self) -> Any:
        ...
        object every single time this function is called.
        """
        raise IsNotImplemented("KerasModelTemplate.new_model()")
